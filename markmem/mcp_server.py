...
Run: `markmem serve --repo <path>` or `python -m markmem.mcp_server` with
MARKMEM_REPO set. Requires the [mcp] extra; import stays lazy so the core
...
def build_server(repo_path: str):
    ...
    def wiki_supersede(page_id: str, old_claim_id: str, new_text: str) -> str:
        ...
        memory.git.commit_all(f"markmem: supersede {old_claim_id} on {page_id}")
...
def main(repo_path: Optional[str] = None) -> None:
    repo_path = repo_path or os.environ.get("MARKMEM_REPO", ".")
    try:
        server = build_server(repo_path)
    except ImportError as e:
        raise SystemExit(
            f"MCP server needs the [mcp] extra: pip install markmem[mcp] ({e})")
    server.run()
