"""`markmem` CLI — init, ingest, search, governance, lifecycle, interop, evals."""
...
app = typer.Typer(help="MarkMem — git-native memory layer (markdown + git, Mem0-shaped API).",
...
def init(path: str = typer.Argument(".", help="Directory to initialize")):
    ...
    m = _memory(path)
    m.close()
    console.print(f"[green]Initialized markmem repo at[/green] {Path(path).resolve()}")
...
def reindex(repo: str = _REPO_OPT):
    """Rebuild .markmem/index.db entirely from the markdown (the invariant)."""
    with _memory(repo) as m:
        n = m.reindex()
...
def doctor(repo: str = typer.Option(None, "--repo", "-r")):
    ...
    if repo:
        ...
        r = Repo(repo)
        checks.append((f"repo at {r.root}", r.is_initialized, "run `markmem init`"))
        if r.is_initialized:
            try:
...
def import_cmd(path: str = typer.Argument(...),
               ...
               repo: str = _REPO_OPT):
    """Import memory from Mem0's export or MarkMem JSONL."""
    ...
    with _memory(repo) as m:
        if from_ == "jsonl":
            ...
            raise typer.Exit(1)
        m.git.commit_all(f"markmem: import {len(written)} page(s) from {from_}")
        m.reindex()
...
def claim(page_id: str = typer.Argument(...),
    ...
    with _memory(repo) as m:
        ...
        if add or close:
            ...
            m.git.commit_all(f"markmem: claim edit on {page_id}")
...
def serve(repo: str = _REPO_OPT):
    """Run the MCP server (stdio) — needs `pip install markmem[mcp]`."""
...
def api(repo: str = _REPO_OPT,
    ...
    """Run the FastAPI REST server — needs `pip install markmem[api]`."""
