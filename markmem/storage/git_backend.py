...
def writer_lock(repo_root: Path, timeout: float = 30.0) -> FileLock:
    lock_dir = repo_root / ".markmem" / "locks"
...
class SubprocessGit:
    ...
    def init(self) -> None:
        if not git_available():
            raise GitError("git executable not found on PATH — MarkMem requires git (see `markmem doctor`)")
    ...
    def _ensure_identity(self) -> None:
        if self._identity_checked:
            return
        for key, default in (("user.name", "markmem"), ("user.email", "markmem@localhost")):
