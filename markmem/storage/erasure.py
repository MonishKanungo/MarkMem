...
def forget_user(repo: Repo, git: SubprocessGit, user_id: str,
    ...
    sha = git.commit_all(f"markmem: forget user {user_id} (mode={mode}, {n_pages} pages)")
    if mode == "rewrite":
    ...
    Ledgers(repo.markmem_dir).record_op("erasure", **tombstone)
...
def merge_users(repo: Repo, git: SubprocessGit, from_user: str, into_user: str) -> int:
    ...
    if profile is not None:
        ...
        if from_user not in page.aliases:
            ...
            repo.write_page(page, body)
    repo.regenerate_index()
    git.commit_all(f"markmem: merge user {from_user} into {into_user} ({moved} pages)")
    Ledgers(repo.markmem_dir).record_op(
