...
The v1 review queue uses JSON files in .markmem/review/. This v2 upgrade makes
...
Architecture:
    Quarantined op → write to staging branch → branch lives in .markmem/staging/
    accept → git merge staging branch into HEAD → delete branch
    reject → delete staging branch (no merge)

Branch naming: markmem/review/<item-id>
    e.g. markmem/review/r-a1b2c3d4e5
...
class StagingReviewQueue:
    ...
    1. A git branch named markmem/review/<item-id>
    2. The proposed page written to that branch
    3. A metadata file .markmem/staging/<item-id>.json with the op + reasons

    Reviewers can inspect with: git diff main markmem/review/<item-id>
    """
    ...
    def __init__(self, repo: Repo, git: SubprocessGit, ledgers: Ledgers,
        ...
        self.schema = schema
        self.staging_dir = repo.markmem_dir / "staging"
        ...
        try:
            ...
            if status:
                self.git._run("commit", "-q", "-m",
                              f"markmem: review proposal — {op.type}/{op.title or 'page'}")
        finally:
            ...
            if self._branch_exists(branch):
                try:
                    ...
                    self.git._run("merge", "--no-ff", branch,
                                  "-m", f"markmem: review-accept {item_id}")
                    ...
                    self.git._run("commit", "-q", "-am",
                                  f"markmem: review-accept {item_id} (fallback)")
