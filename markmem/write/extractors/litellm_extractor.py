...
Together, NVIDIA NIM and ~90 more behind one `completion()` call, so MarkMem
...
class LiteLLMExtractor:
    ...
    def __init__(self, config: Config, ledgers: Optional[Ledgers] = None,
        ...
        try:
            ...
            raise ImportError(
                'LiteLLM extractor needs the extra: pip install "markmem[litellm]"'
            ) from exc
        self._litellm = client or litellm

        self.model = os.environ.get("MARKMEM_LLM_COMPILE_MODEL") or config.llm.compile_model
        self.api_base = getattr(config.llm, "litellm_api_base", None)
        self.ledgers = ledgers
        self._use_tools = supports_tool_use(self.model)

        # LiteLLM is chatty by default; keep MarkMem's logs readable.
