"""FastAPI REST layer — Phase 2 HTTP interface for MarkMem (§12).
...
Run:
    pip install markmem[api]
    markmem api --repo ./my-memory --host 0.0.0.0 --port 8000

Or programmatically:
    from markmem.api import build_app
...
def build_app(repo_path: str | Path, start_worker: bool = True):
    ...
    Args:
        repo_path: Path to the MarkMem memory repo (created if absent).
...
def main(repo_path: Optional[str] = None, host: str = "0.0.0.0",
    ...
    """Entry point for `markmem api` CLI command."""
    try:
        import uvicorn
    except ImportError:
        raise SystemExit(
            "FastAPI REST layer needs the [api] extra: pip install markmem[api]"
        )
    repo = repo_path or os.environ.get("MARKMEM_REPO", ".")
    app = build_app(repo)
