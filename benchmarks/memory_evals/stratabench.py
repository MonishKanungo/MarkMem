"""StrataBench — Strata's own open, hand-authored memory benchmark.
...
StrataBench fixes that. Every question declares:
...
Run:
    python -m benchmarks.memory_evals.stratabench            # offline
    python -m benchmarks.memory_evals.stratabench --with-llm # + EM/F1
"""
...
def _semantic_blocks(packed: str) -> str:
    ...
    as strata/evals/harness.py. An old statement inside a dated what-happened
...
def run_stratabench(work_dir: Path, k: int = 5, llm=None,
                    force_heuristic: bool = True,
                    progress: Callable[[str], None] = print) -> dict:
    """Run the full StrataBench suite. Returns a metrics dict."""
    memory = fresh_memory(work_dir, "stratabench", force_heuristic)
    try:
...
def print_report(r: dict) -> None:
    ...
    c.rule("[bold green]StrataBench — hand-authored, ground-truth benchmark[/bold green]")
    ...
    t2.add_column("metric"); t2.add_column("Strata", justify="right")
...
def main(argv: list[str] | None = None) -> dict:
    ...
    ap = argparse.ArgumentParser(description="StrataBench — Strata's own benchmark")
    ...
    work = Path(tempfile.mkdtemp(prefix="stratabench-"))
    result = run_stratabench(work, k=args.k, llm=llm,
                             force_heuristic=not args.llm_extract)
    print_report(result)

    out = args.out or (Path(__file__).resolve().parents[2] / "benchmarks" /
                       "results" / f"stratabench-{time.strftime('%Y-%m-%dT%H-%M-%S')}.json")
