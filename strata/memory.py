...
class Memory:
    def __init__(self, repo_path: str | Path = "./strata-memory", auto_init: bool = True,
                 config: Optional[Config] = None, start_worker: bool = True,
                 force_heuristic: bool = False):
        self.repo = Repo(repo_path)
        if not self.repo.is_initialized:
            if not auto_init:
                raise FileNotFoundError(f"no strata repo at {self.repo.root} (auto_init=False)")
        ...
        if self.git.commit_all("strata: init repo") is not None:
            log.info("initialized strata repo at %s", self.repo.root)
        if start_worker:
            self.pipeline.start()
    ...
    def update(self, memory_id: str, data: str) -> dict[str, Any]:
        ...
        with writer_lock(self.repo.root):
            ...
            self.git.commit_all(f"strata: human update {memory_id}")
    ...
    def delete(self, memory_id: str, hard: bool = False) -> bool:
        ...
        with writer_lock(self.repo.root):
            if hard:
                ...
                self._on_pages_written([memory_id])
            self.git.commit_all(
                f"strata: {'hard delete' if hard else 'archive'} {memory_id}")
        return True
    ...
    def delete_all(self, user_id: str, hard: bool = False) -> int:
        ...
        with writer_lock(self.repo.root):
            ...
            if changed:
                ...
                self.git.commit_all(f"strata: archive all pages of {user_id} ({n})")
        return n
    ...
    def reset(self) -> None:
        ...
        with writer_lock(self.repo.root):
            ...
            for name in ("queue.db", "index.db", "review"):
                target = self.repo.strata_dir / name
                if target.is_dir():
                    shutil.rmtree(target)
                elif target.exists():
                    target.unlink()
            Repo.scaffold(self.repo.root)
            self.git.commit_all("strata: reset (wiki + raw wiped)")
        self.indexer = Indexer(self.repo.strata_dir)
