...
Durability: the queue is a SQLite table in .strata/queue.db (WAL). Rows stuck
...
class WritePipeline:
    ...
    def _process_rows(self, rows: list[tuple[int, str]]) -> int:
        ...
        if written:
            ...
            self.git.commit_all(
                f"strata: compile {len(rows) - failed} raw entr"
    ...
    def start(self) -> None:
        if self._worker is not None and self._worker.is_alive():
            return
        self._stop.clear()
        self._worker = threading.Thread(target=self._run, name="strata-writer", daemon=True)
        self._worker.start()
