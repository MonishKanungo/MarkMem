"""LLM extractor — NVIDIA NIM (Nemotron etc.) via tool-calling, pydantic-validated.

Mirrors AzureOpenAIExtractor exactly (same retry/validation/batch behavior);
only the client construction and env vars differ, since NVIDIA NIM exposes
the same OpenAI-compatible chat.completions + tool-calling surface.
"""
...
class NvidiaExtractor:
    ...
    def __init__(self, config: Config, ledgers: Optional[Ledgers] = None, client: Any = None):
        if client is None:
            ...
            self.client = client
        self.model = os.environ.get("NEMOTRON_MODEL", "meta/llama-3.1-8b-instruct")
    ...
    def _call(self, messages: list[dict]) -> Any:
        ...
        for attempt in range(3):
            try:
                ...
                last_err = e
                log.warning("NVIDIA NIM rate limit hit (attempt %d/3), waiting 5s...", attempt + 1)
                time.sleep(5.0)
            except (openai.APIConnectionError, openai.APIStatusError) as e:
                last_err = e
                log.warning("NVIDIA NIM API/connection error (attempt %d/3), waiting 2s: %s", attempt + 1, e)
                time.sleep(2.0)
        raise last_err
