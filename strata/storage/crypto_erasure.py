...
Key store backends:
- FileKeyStore  — local .strata/keys/ directory (default, no deps)
- EnvKeyStore   — keys from environment variables (CI/CD, simple deployments)

Usage:
    from strata.storage.crypto_erasure import CryptoErasureManager
...
NOTE: This module is opt-in. Strata works without it (plaintext files in git).
Enabling crypto-shred is a deployment decision — mixing encrypted and plaintext
files in the same repo is not supported.

Install: pip install strata-memory[crypto]  (adds cryptography package)
"""
...
class FileKeyStore:
    """DEKs stored as files under .strata/keys/<user_id>.key.
...
class EnvKeyStore:
    """DEKs from environment variables — STRATA_DEK_<USER_ID>=<hex>.
    ...
    def _env_key(self, user_id: str) -> str:
        return "STRATA_DEK_" + user_id.upper().replace("/", "_").replace("-", "_")
...
def _require_crypto():
    try:
        ...
        raise ImportError(
            "Crypto-shred needs the [crypto] extra: pip install strata-memory[crypto]"
        ) from exc
...
class CryptoErasureManager:
    ...
    def __init__(self, repo_root: Path, key_store: Optional[KeyStore] = None):
        ...
        self.key_store = key_store or FileKeyStore(self.root / ".strata")
